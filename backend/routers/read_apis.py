...
def read_codes(auth_token: str, code_id: int, db: Session = Depends(get_db)):
    ...
    # Find the entry by code_id
    db_code = (
        db.query(models.CodeSnippet)
        .filter(models.CodeSnippet.code_id == code_id)
        .first()
    )
    if not db_code:
...
def read_journal_by_tags(auth_token: str, tag_id: int, db: Session = Depends(get_db)):
    ...
    journals = (
        db.query(models.journals_and_tags)
        .filter(models.journals_and_tags.c.tag_id == tag_id)
        .all()
    )
    if not journals:
...
def get_tags_by_journal(
    ...
    tags = (
        db.query(models.journals_and_tags)
        .filter(models.journals_and_tags.c.journal_id == journal_id)
        .all()
    )
    if not tags:
