...
def read_entries(auth_token: str, page_number: int, db: Session = Depends(get_db)):
    """
    ...
    db_entry = db.query(models.Entry).filter(models.Entry.page_number == page_number).first()
    if not db_entry:
